...
*/
    ...
    public static void correctAnswer(char[][] answers, char[] keys) {
        int counter=0;

        for(int i=0;i<answers.length;i++){//0'dan ogrenci sayisina kadar donguye aldik

            for(int j=0;i<answers[i].length;i++){//0'dan ogrenci cevap sayisina kadar donguye aldik

                if(answers[i][j]==keys[j]){//ogrencinin cevabini cevap anahtariyla kontrol ediyoruz
                    counter++;
                }

            }
            System.out.println(i+1+"nolu student's answer number:"+counter);
        }
    }
}
